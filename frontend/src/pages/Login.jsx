...
import { Link, ShieldCheck, ArrowRight } from 'lucide-react'; // Naya ArrowRight icon add kiya hai
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user && user.email.endsWith('@gmail.com')) {
        ...
        // FIX: Yahan popup nahi dikhana, bas logged in state true karni hai
        setIsLoggedIn(true);
        setUserName(user.displayName.split(' ')[0]); // Sirf First Name
      }
    });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (userEmail.endsWith('@gmail.com')) { 
        ...
        // FIX: Loading roko, state true karo, popup MAT dikhao
        setIsLoggedIn(true);
        ...
        <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl flex flex-col items-center">
          ...
          {isLoggedIn ? (
            ...
            <div className="flex flex-col items-center gap-6 w-full">
              ...
              <button 
                onClick={() => setShowRules(true)} // IS BUTTON SE POPUP KHULEGA
            ...
            // STATE 2: AGAR USER LOGGED OUT HAI (Tumhara Purana Google Button)
            <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in duration-300 relative">
            ...
            <div className="flex gap-4">
              <button 
                onClick={() => setShowRules(false)} // NAYA: Cancel button add kiya taaki user wapas peeche ja sake
